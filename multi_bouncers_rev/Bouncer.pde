class Bouncer {
  ...
  void check_edges() {
    ...
    // 수직벡터로 반사되는 속도벡터를 구하는 부분
    PVector N = new PVector(pos.x - min_x, pos.y - min_y); // 수직벡터
    N.normalize(); //크기가 1인 수직벡터
    PVector D = new PVector(vel.x, vel.y);
    // 반사벡터 = D - 2 ( D 내적 N ) N
    PVector ref = D.add( N.mult( -2 * (N.dot(D)) ) ); //반사되는 속도벡터

    if (dist(min_x, min_y, pos.x, pos.y) < r * 0.5) {
      if (!collided) {
        vel.x = ref.x;
        vel.y = ref.y;
        collided = true; // 충돌! (없는 것보다는 오류가 적은 것 같습니다)
      }
    }else{
      collided = false; // 다시 충돌 가능
    }
  ...
  void update() {
    ...
    check_edges();
    // 가속도 더해주는 부분
    acc.set(mouseX-pos.x, mouseY-pos.y); // 마우스의 위치를 향한 벡터
    acc.normalize(); // 크기를 1로 바꾸어준다
    acc.mult(0.1); // 0.1의 크기를 가지는 가속도
    vel.add(acc); // 가속도를 속도에 더해준다.
    vel.limit(5);
    pos.add(vel);
    strokeWeight(10);
    // 속도 그려주는 선
